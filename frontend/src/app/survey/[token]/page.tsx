...
export default function SurveyPage() {
  ...
  const validateToken = async () => {
    try {
      ...
      // Validate token with backend GET endpoint
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_API_URL}/api/culture-assessment/survey/validate-token?token=${token}`,
        {
          method: 'GET',
          headers: {
      ...
      if (!response.ok) {
        ...
        if (errorData.code === 'ALREADY_COMPLETED') {
          setError('This survey has already been completed.');
        } else if (errorData.code === 'EXPIRED') {
          setError('This survey link has expired. Please contact your administrator.');
        } else if (errorData.code === 'INVALID_TOKEN') {
          setError('Invalid survey link. Please check the URL.');
        } else {
          setError(errorData.error || 'This survey link is invalid or has expired.');
        }
        setValidToken(false);
        return;
      }
      ...
      if (validationData.success && validationData.valid) {
        ...
        setError('Survey validation failed');
        setValidToken(false);
      }
