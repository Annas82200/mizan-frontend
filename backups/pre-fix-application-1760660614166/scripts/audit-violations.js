...
const MOCK_DATA_PATTERNS = [
  ...
  /(?:placeholder|temp_?data|fake_?data|sample_?data|test_?data|dummy_?data|mock_?(?:user|employee|company|tenant)|todo_?implement|fixme|tbd|coming_?soon)/i,
  /\b(mock|dummy|placeholder|temp|sample|test|fake|example)\w*/gi,
  /mockResponse/i,
  /fake.*data/i,
  /dummy.*data/i, // AUDIT_RULE: Pattern to detect placeholder data - NOT a violation itself
  /placeholder.*data/i,
  /sample.*data/i,
...
const TYPESCRIPT_VIOLATIONS = [
  ...
  /:\s*any\b/,
  /as\s+any\b/,
  // Define strict TypeScript interfaces for audit violation data
...
function processViolation(violation: ViolationMatch): FileViolation {
  return {
